...
- Folder-backed sync roots write `tglfs:sync-manifest` version 2. Readers must continue to parse sync-manifest v1 records, where no `folderId` exists, but newly initialized or upgraded sync roots must link to a `tglfs:folder` record through `folderId`.
- TGLFS folder structure uses one global folder protocol version, currently `TGLFS_FOLDER_PROTOCOL_VERSION = 2`. The `tglfs:folder` message caption and its attached plain JSON `tglfs:folder-entries` payload MUST use the same folder protocol version. Do not split these into independently bumped caption/payload versions unless the user explicitly approves that compatibility model.
- Current folder writes must keep the Telegram message text small: write folder identity, parent/root links, name/path, revision, and entry hash in the `tglfs:folder` caption, and store directory entries in the folder message's plain JSON attachment. Do not store large folder entry maps in Telegram message text.
- Readers must preserve compatibility with previously written folder records: `tglfs:folder` v1 records and separate `tglfs:folder-manifest` v1 messages remain readable. New writes should produce folder v2 attachment-backed entries rather than new v1 manifest text messages when a folder card exists.
- Folder entries reference immutable `tglfs:file` blobs by UFID and child folders by `folderId`; do not move blob/chunk metadata into folder records or folder-entry attachments.
